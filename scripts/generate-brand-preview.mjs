...
const symbol = `<path d="${pulsePath}"/>`;
const mark = (x, y, size, color) =>
  `<svg x="${x}" y="${y}" width="${size}" height="${size}" viewBox="0 0 64 64" fill="${color}">${symbol}</svg>`;
const icons = (x, y, color, bg) =>
  [16, 24, 32, 48, 64]
    .map(
      (s, i) =>
        `<rect x="${x + i * 80}" y="${y + 64 - s}" width="${s}" height="${s}" rx="${s / 4}" fill="${bg}"/>${mark(x + i * 80, y + 64 - s, s, color)}`,
    )
    .join("");
...
<g font-family="Arial,sans-serif" font-size="12" letter-spacing="3"><text x="48" y="54" fill="#5c5c5c">PULSE / SIGNAL MARK</text><text x="648" y="54" fill="#a3a3a3">ONE MARK. EVERY SURFACE.</text></g>
...
<g font-family="Arial,sans-serif" font-size="11" fill="#707070"><text x="48" y="660">16 / 24 / 32 / 48 / 64 px</text><text x="648" y="660" fill="#a3a3a3">Connected systems. One clear signal.</text></g></svg>`;
